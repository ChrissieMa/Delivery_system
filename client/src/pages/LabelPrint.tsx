...
import { Printer, ArrowLeft, FileDown, Loader2, Share2 } from "lucide-react";
...
export default function LabelPrint() {
  ...
  const labels: DeliveryLabelData[] = sourcePackages.map((pkg: any, index: number) => ({
    key: pkg.id,
  ...
  const sharePdf = async (pdf: PreparedPdf) => {
    const shareData: ShareData = { files: [pdf.file], title: "LKS Product Labels" };
    try {
      if (
  ...
  const exportFixedPdf = async () => {
    ...
    try {
      const blob = await createLabelsPdf(labels);
      ...
      const nextPdf = { blob, file, filename };
